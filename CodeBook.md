...
<TABLE border=1>  
<caption> <u>Table #1</u>: Human Activity Recognition Using Smartphones Data Set Variable Names </caption>  
...
The defining characteristic of a "tidy tool" defined by Hadley Wickham in his presentation entitled ["Tidy data & tidy tools"](http://vimeo.com/33727555) is that it operates on tidy data and does not change its format. Therefore, I generated a visualization of one of the variables contained in the data set generated by `run_analysis.R` using a tidy tool (i.e. ggplot2). Since no errors or warnings occurred during the generation of this plot shown in Figure #1, this result suggest that `run_analysis.R` generated a tidy data set.   
